...
class ModelTrainer(QThread):
    training_finished = pyqtSignal()  # Signal emitted when training is complete, so that future message cna be displayed

    def __init__(self, main_window):
        super().__init__()
        self.main_window = main_window  # Referencing the main window that holds the model
...
# Import the UI files I generated using PyQt Designer previously
from ui_Splash import Ui_SplashScreen
from ui_MainWindow2 import Ui_MainWindow
from ui_Settings2 import Ui_Settings

# Path to the settings file where my user preferences are saved
...
def load_settings():
    try:
        ...
        # If the settings file doesn't exist, return default settings, prevent errors from occuring
        return {
            "wait_time": "5",
            "sessions": [],
            "categories": {},
            "whitelisted_sites": [],
            "blacklisted_sites": [],
            "productive_sites": [],  # List of productive sites in settings file
...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        self.currently_in_session = False  # Indicates whether the user is in a session, aka current time is in schedule
        self.model_trained = False  # Indicates whether the machine learning model is trained yet
        ...
        # Unproductive category names with example site links, just in case I later want to train models with website names
        self.category_sites = {
            "Social Media": ["facebook.com", "twitter.com", "instagram.com", "discord.com"],
            "Games": ["store.epicgames.com", "store.steampowered.com", 
                      "poki.com", "minesweeper.online","nytimes.com/games/wordle", "nytimes.com/crosswords/game/mini",
                      "playhop.com","coolmathgames.com","aol.com/games","solitaired.com"],
            "Video Entertainment": ["youtube.com", "sky.com","en.tinyzone-tv.com","vimeo.com/watch","netflix.com/hk-en/browse/genre/839338","ondisneyplus.disney.com","amazon.com/gp/video/storefront"],
            "Visual Entertainment": ["globalcomix.com", "mangadex.org", "manganato.com", "anime-planet.com", "readallcomics.com"],
            "Forums": ["reddit.com","reddit.com/r/popular","reddit.com/r/all" "quora.com", "threads.net/?hl=en","reddit.com/r/all/new"],
            "Online Shopping": ["amazon.com", "alibaba.com", "aliexpress.com", "zalora.com", "eBay.com", "taobao.com", "hktvmall.com","zalora.com.hk/s/women","hktvmall.com/hktv/en/","myntra.com"],
        }
    ...
    # Capture a screenshot of the entire screen, to extract text from content viewed
    ...
    def detect_unproductive_activity(self):
        ...
        if screen_text.strip():
            ...
            self.unproductive_flag = self.predict_unproductive(screen_text)  # Predict if the content extracted is unproductive
            ...
            if self.unproductive_flag:
                if self.override_delay:
                    ...
                    if not self.unproductive_timer.isValid():
                        ...
                        self.unproductive_timer.start()  # Start the timer for unproductive activity, recording consistently unproductive viewing from users
    ...
    # Show a modal dialog with a warning message that blocks screen
    @QtCore.pyqtSlot()
